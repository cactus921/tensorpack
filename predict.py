...
def do_predict(pred_func, input_file):
    ...
    img_resized = cv2.resize(img, (500, 375))
    results = predict_image(img_resized, pred_func)
    dict = {}
    dict['rcnn_predictions'] = []
    for dr in results:
        box = {'xmin': str(dr.box[0]), 'ymin': str(dr.box[1]), 'xmax': str(dr.box[2]), 'ymax': str(dr.box[3])}
        dict['rcnn_predictions'].append({'box': box, 'score': str(dr.score), 'class': str(dr.class_id) })
...
# def my_predict(image_file, config):
#     print("AAAA")
...
#     img_resized = cv2.resize(img, (500, 375))
#     results = predict_image(img_resized, predictor)
#     return results